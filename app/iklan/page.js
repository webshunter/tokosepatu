...
export default function Profile({ params }) {
    ...
    useEffect(() => {
        ...
        (async function (user) {
            if(user){
                console.log(user)
                let data = await fetch('/pages/api/produk?limit=500&start=0&uid_user=' + user.uniqid + ``);
                        ...
                        <div className="block w-full">
                            <div className="flex md:block ml-auto mr-auto w-full md:w-[50%]">
                                <div className="relative " style={{ width: "fit-content" }}>
                                    {user? (user.avatar ?
                                        (
                                                <figure className="w-[56px] md:w-[106px] h-[56px] md:h-[106px] bg-[50%] bg-cover rounded-full m-0" style={{ backgroundImage: `url('${user.avatar ? user.avatar : `https://ui-avatars.com/api/?size=200&background=25D366&name=` + (user.fullname ? user.fullname : '').split(' ').join('+') }')` }}></figure>
                                        ) :
                                        (
                                                <figure className="w-[56px] md:w-[106px] h-[56px] md:h-[106px] bg-[50%] bg-cover rounded-full m-0" style={{ backgroundImage: `url('${session? session.user.image:``}')` }}></figure>
                                        ) ) :<></>}
                                </div>
                            </div>
                            <div className="flex items-center overflow-hidden whitespace-nowrap text-ellipsis">
                                <span className="text-[20px] leading-[30px] capitalize w-full font-bold">{user ? user.fullname : "-"}</span>
                            </div>
                                ...
                                <div className="my-[20px]">
                                    <div className="flex items-center">
                                        ...
                                        <div className="m-[4px] ml-[5px]">
                                            <span>Anggota sejak {JoinDate(user?user.tgldaftar:new Date())}</span>
                                        </div>
                                    </div>
                                    {about ? (
                                        <div className="flex items-center">
