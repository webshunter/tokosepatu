...
const IklanTerkait = ({data}) => {
    ...
    const { data: seconData } = useSWR(`/pages/api/produk?order=uniqid&ascdesc=DESC&limit=8&start=0&approval=1&slug2=${properti.slug2.replace(/\&/g, '~')}&prov=${properti.prov}&n=${btoa(JSON.stringify([['slug',properti.slug]]))}`, fetcher)
    return <>
    <div className='p-5'>
