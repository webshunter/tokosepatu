...
export default function LoadData(props) {
  const { data } = useSWR('/pages/api/produk?limit=21&start=0&uid_user=ID-USER-1698900322534', fetcher)
  const [dataListing, setDataListing] = useState([]);
  useEffect(() => {
    document.getElementById('search').value = '';
    (async function () {
      let data = await fetch(`/pages/api/produk?limit=21&start=0&uid_user=ID-USER-1698900322534`);
